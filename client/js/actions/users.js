...
export function validateEmail(validateEmailToken) {
  ...
  const request = axios.get(`http://localhost:3000/api/v1/validateEmail/${validateEmailToken}`);
...
export function meFromToken(tokenFromStorage) {
  ...
  const request = axios({
    method: 'get',
    url: `http://localhost:3000/api/v1/me/from/token?token=${tokenFromStorage}`,
    headers: {
...
export function signInUser(formValues) {
  const request = axios.post(`http://localhost:3000/api/v1/security/login`, formValues);
